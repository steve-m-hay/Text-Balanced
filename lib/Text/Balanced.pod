...
=head1 SYNOPSIS
 ...
 # Extract the initial substring of $text which is bounded by
 # an HTML/XML tag.
...
which matches any standard HTML/XML tag is used.
...
The various options which can be specified are:
...
which must I<not> appear within the tagged text.
...
The list reference contains one or more strings specifying patterns
which are I<not> be be treated as nested tags within the tagged text
...
after the tag (up to the first line which is either empty or contains
...
substring which matched a tagged text (including the start and end
...
builds a regex which matches a string delimited by any one of the
...
If no escape char is specified, '\' is used for all delimiters.
...
C<extract_tagged> found a nested tag which appeared in the "reject" list
(and the failure mode was not "MAX" or "PARA").

=item C<Found unbalanced nested tag: %s>

C<extract_tagged> found a nested opening tag which was not matched by a
