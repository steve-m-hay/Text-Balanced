...
In a scalar context, just the extracted string is returned (again, the
...
which matched a quotelike operation (or C<undef> on failure). In a void
context, the input text has the same substring (and any specified
prefix) removed.
...
In a list context, all the functions return C<('',$original_text,'')>
...
diagnostic messages is returned in the standard C<$@> variable:
